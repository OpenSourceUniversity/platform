...
contract Course {
    ...
    struct CourseStruct {
        ...
        address[] graduatStorage;     // graduate storages containing all the users who compleated this course
    ...
    // CRUD cours (if number of courses > 1 than its a degree)
    function addCourse (
        address _verifier,
        uint _numberRetings,
        uint _averageRating,
        ...
        returns (bool)
    {
        ...
        course[course.length-1].numberRetings = _numberRetings;
        course[course.length-1].averageRating = _averageRating;
        ...
        course[course.length-1].graduatStorage = _graduatStorage;
    ...
    function getNrOfRaters (uint _index) public constant returns (uint) {
        require(_index < course.length);
        return course[_index].numberRetings;
    }
    ...
    function getGraduatesCount(uint _index) public constant returns (uint) {
        require(_index < course.length);
        return course[_index].graduatStorage.length;
    }
      ...
      i[0] = 0;
          for (i[0] = 0; i[0] < course.length; i[0]++) {
              if (course[i[0]].creator == tx.origin) {
                  ...
                  course[i[0]].numberRetings = _numberRetings;
                  ...
                  course[i[0]].graduatStorage = _graduatStorage;
