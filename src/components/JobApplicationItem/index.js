import React from 'react';
import { Button, Feed, Dropdown, Grid, Header, Icon } from 'semantic-ui-react';
import { connect } from 'react-redux';
import { withRouter } from 'react-router-dom';
import SkillItem from 'components/SkillItem';
import changeApplicationState from '../../util/jobApplication/changeApplicationState';


class JobApplicationItem extends Dropdown.Item {
  isVerified(certificate) {
    for (let i = 0; i < certificate.verifications.length; i += 1) {
      if (certificate.verifications[i][certificate.verifications[i].length - 1].state === 'verified') {
        return true;
      }
    }
    return false;
  }

  renderState(state) {
    if (state === 'rejected') {
      return (
        <div>
          <Icon color="red" name="close" /> Rejected
        </div>);
    }
    return (
      <div>
        <Icon color="green" name="checkmark" /> Approved
      </div>);
  }

  renderMatchedSkills() {
    const { certificates } = this.props.application;
    const jobSkills = this.props.application.job.skills;
    certificates.sort((a, b) => b.verified);
    let verifiedSkills = [];
    let notVerifiedSkills = [];
    const skills = [];
    for (let i = 0; i < certificates.length; i += 1) {
      if (this.isVerified(certificates[i])) {
        verifiedSkills = verifiedSkills.concat(certificates[i].skills);
      } else {
        notVerifiedSkills = notVerifiedSkills.concat(certificates[i].skills);
      }
    }
    verifiedSkills = [...new Set(verifiedSkills.map(e => e.name))];
    notVerifiedSkills = [...new Set(notVerifiedSkills.map(e => e.name))];
    notVerifiedSkills = notVerifiedSkills.filter(el => !verifiedSkills.includes(el));
    verifiedSkills = verifiedSkills.filter(el => jobSkills.map(e => e.name).includes(el));
    notVerifiedSkills = notVerifiedSkills.filter(el => jobSkills.map(e => e.name).includes(el));
    for (let i = 0; i < verifiedSkills.length; i += 1) {
      skills.push({
        have_icon: true, check: true, name: verifiedSkills[i], basic: true,
      });
    }
    for (let i = 0; i < notVerifiedSkills.length; i += 1) {
      skills.push({
        have_icon: true, check: false, name: notVerifiedSkills[i], basic: true,
      });
    }
    if (skills.length === 0) {
      return 'There is no matching skills';
    }
    return skills.map((skill, index) => (
      <SkillItem skill={skill} key={index} />
    ));
  }

  render() {
    const { application } = this.props;
    const avatarPlaceholder = 'data:image/jpeg;base64,';
    return (
      <Feed.Event
        style={{
          padding: '15px',
          borderBottom: '1px solid #ccc',
        }}
      >
        <Feed.Label
          onClick={
            application ?
              () => { this.props.history.push(`/view-profile/learner/${application.issuer_eth_address}/`); } :
              null
          }
          style={{
            cursor: application ? 'pointer' : 'auto',
          }}
        >
          <img
            src={
              application.issuer_profile.learner_avatar ?
                `https://ipfs.io/ipfs/${application.issuer_profile.learner_avatar}` :
                avatarPlaceholder}
            alt=""
          />
        </Feed.Label>
        <Feed.Content style={{ marginTop: 0 }} >
          <Feed.Summary>
            <Grid>
              <Grid.Column width={4}>
                <Header
                  onClick={
                    application ?
                      () => { this.props.history.push(`/view-profile/learner/${application.issuer_eth_address}/`); } :
                      null
                  }
                  style={{
                    cursor: application ? 'pointer' : 'auto',
                  }}
                >
                  {`${application.issuer_profile.first_name} ${application.issuer_profile.last_name}`}
                  <Header.Subheader>
                    {this.props.application.issuer_profile.learner_country}
                  </Header.Subheader>
                </Header>
              </Grid.Column>
              <Grid.Column width={4}>
                {this.renderMatchedSkills()}
              </Grid.Column>
              <Grid.Column width={4} style={{ textAlign: 'center' }}>
                <Header
                  onClick={
                    application ?
                      () => { this.props.history.push(`/job-page/${application.job.id}/`); } :
                      null
                  }
                  style={{
                    cursor: application ? 'pointer' : 'auto',
                  }}
                >
                  {application.job.title}
                </Header>
              </Grid.Column>
              <Grid.Column width={4} style={{ textAlign: 'center' }}>
                {
                  application.state === 'rejected' || application.state === 'approved' ?
                    this.renderState(application.state) :
                    <div>
                      <Button
                        name="approve"
                        inverted
                        color="green"
                        onClick={
                          (e, { name }) => {
                            const { id } = application;
                            const address = application.issuer_eth_address;
                            this.props.changeApplicationState(id, name, address);
                          }
                        }
                      >
                        APPROVE
                      </Button>
                      <Button
                        name="reject"
                        inverted
                        color="red"
                        onClick={
                          (e, { name }) => {
                            const { id } = application;
                            this.props.changeApplicationState(id, name, null);
                          }
                        }
                      >
                        REJECT
                      </Button>
                    </div>
                }
              </Grid.Column>
            </Grid>
          </Feed.Summary>
        </Feed.Content>
      </Feed.Event>
    );
  }
}


function mapStateToProps(state) {
  return {
    notifications: state.notification.notifications,
    unreadNotificationsCount: state.notification.unreadNotificationsCount,
    isFetching: state.notification.isFetching,
    nextUrl: state.notification.nextUrl,
  };
}

function mapDispatchToProps(dispatch) {
  return {
    changeApplicationState(id, state, ethAddress) {
      dispatch(changeApplicationState(id, state, ethAddress));
    },
  };
}


export default connect(mapStateToProps, mapDispatchToProps)(withRouter(JobApplicationItem));
