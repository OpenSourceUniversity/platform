...
import { Feed, Dropdown, Grid, Header, Icon } from 'semantic-ui-react';
import { connect } from 'react-redux';
import { withRouter } from 'react-router-dom';
import SkillItem from 'components/SkillItem';


class JobApplicationLearnerItem extends Dropdown.Item {
  ...
  renderState(state) {
    ...
    return (
      <div>
        <Icon color="blue" name="time" /> Requested
      </div>);
  }

  renderMatchedSkills() {
    const { certificates } = this.props.application;
    const jobSkills = this.props.application.job.skills;
    ...
    verifiedSkills = verifiedSkills.filter(el => jobSkills.map(e => e.name).includes(el));
    notVerifiedSkills = notVerifiedSkills.filter(el => jobSkills.map(e => e.name).includes(el));
    ...
    return (
      ...
      >
        <Feed.Label
          onClick={
            application ?
              () => { this.props.history.push(`/view-profile/business/${application.company_eth_address}/`); } :
              null
          }
          style={{
        ...
        >
          <img
            src={
              application.company_profile.company_logo ?
                `https://ipfs.io/ipfs/${application.company_profile.company_logo}` :
                avatarPlaceholder}
            alt=""
          />
        </Feed.Label>
          ...
          <Feed.Summary>
            <Grid>
              <Grid.Column width={4}>
                ...
                >
                  {application.company_profile.company_name}
                  <Header.Subheader>
                    {this.props.application.company_profile.company_country}
                  </Header.Subheader>
                </Header>
              </Grid.Column>
              <Grid.Column width={4}>
                {this.renderMatchedSkills()}
              </Grid.Column>
              <Grid.Column width={4} style={{ textAlign: 'center' }}>
                <Header
                  onClick={
                    application ?
                      () => { this.props.history.push(`/job-page/${application.job.id}/`); } :
                      null
                  }
                  style={{
                    cursor: application ? 'pointer' : 'auto',
...
export default connect(mapStateToProps)(withRouter(JobApplicationLearnerItem));
