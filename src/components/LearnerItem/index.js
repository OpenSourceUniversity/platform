import React from 'react';
import PropTypes from 'prop-types';
import { withRouter } from 'react-router-dom';
import { Card, Button, Grid, Segment, Header, Divider } from 'semantic-ui-react';


class LearnerItemWithoutRouter extends React.Component {
  static propTypes = {
    history: PropTypes.object.isRequired,
  }

  renderRating(ratingNumb) {
    return (
      <div className="ui accurate star widget inline" style={{ marginRight: '10px' }}>
        <div className="highlight" style={{ width: `${(ratingNumb / 5) * 100}%` }} />
      </div>);
  }

  render() {
    const avatarPlaceholder = 'data:image/jpeg;base64,';

    return (
      <Card fluid style={this.props.isNotList ? { height: '400px' } : null} onClick={() => { this.props.history.push(`/view-profile/learner/${this.props.learner.user.username}/`); }}>
        <Card.Content>
          <Grid>
            <Grid.Column mobile={16} tablet={8} computer={4}>
              <Segment
                textAlign="center"
                circular
                className="profilePicSegment"
                style={{
                  maxWidth: 125,
                  maxHeight: 125,
                  backgroundImage: `url(${this.props.learner.learner_avatar ?
                    `https://ipfs.io/ipfs/${this.props.learner.learner_avatar}` :
                    avatarPlaceholder})`,
                  backgroundRepeat: 'no-repeat',
                  backgroundSize: 'contain',
                  backgroundPosition: 'center center',
                }}
              />
            </Grid.Column>
            <Grid.Column mobile={16} tablet={8} computer={12}>
              <Header size="large" style={{ textAlign: 'center' }}>
                {this.props.learner.full_name}
              </Header>
              <p>
              Certificates: {this.props.learner.certificates_count}
              </p>
              <Divider clearing />
              <p style={{ wordBreak: 'break-word' }}>
                {this.props.learner.learner_about ? `${this.props.learner.learner_about.substring(0, 100)}...` : '-'}
              </p>
            </Grid.Column>
          </Grid>
        </Card.Content>
        <Card.Content extra>
          <Button name="course-page" onClick={() => { this.props.history.push(`/view-profile/learner/${this.props.learner.user.username}/`); }}>
            Show Profile
          </Button>
        </Card.Content>
      </Card>
    );
  }
}

const LearnerItem = withRouter(LearnerItemWithoutRouter);

export default LearnerItem;
